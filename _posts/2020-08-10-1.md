...
<b>证明</b>&nbsp;&nbsp;&nbsp; 根据定义，我们有$\sum\limits_{n=m}^\infty a_n$收敛当且仅当$(S_N)_{N=m}^\infty$收敛，$S_N=\sum\limits_{n=m}^N a_n.$ 因此$(S_N)_{N=1}^\infty$是一个柯西序列，也即$$\forall\varepsilon>0, \exists N \geq m\ \text{s.t.}\ |S_q-S_{p-1}|=\left|\sum_{n=p}^q a_n\right| \leq \varepsilon, \forall p, q \geq N\ \ \square$$
</p>
</body>
