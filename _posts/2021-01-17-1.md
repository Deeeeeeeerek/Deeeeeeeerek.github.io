...
在证明之前，我们先从不同的角度理解这一引理. 从几何的角度，我们可以将这一引理理解为$A$的行向量与$\mathbf{c}$的位置关系. 从线性代数的角度，$$A_{m \times n}=\begin{pmatrix}\mathbf{a}_1^T \\ \vdots \\ \mathbf{a}_m^T\end{pmatrix}, \mathbf{a}_i \in \mathbb{R}^n$$ 那么
