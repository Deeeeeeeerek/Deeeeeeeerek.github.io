...
Suppose we have $K$ trees, then $$\widehat{y}_i=\sum\limits_{k=1}^Kf_k(x_i), f_k\in\mathcal{F},$$ where $\mathcal{F}$ is the space of regression trees, $f_k(x_i)$ is the $i$th sample's weight of leaf on the $k$th tree.

Similarly, the objective function is $$\mathrm{Obj}=L(\Theta)+\Omega(\Theta)=\sum\limits_{i=1}^nl(y_i, \widehat{y}_i)+\sum\limits_{k=1}^K\Omega(f_k).$$
...
# 5. Stacking
***
Stacking uses $[f_1(x), ..., f_B(x)]$ as features to fit a meta learner.

补充：
GBM: how to learn a tree with grad different objectives.

XGBoost: Regularization - trees cannot be too deep. w cannot vary too much.
